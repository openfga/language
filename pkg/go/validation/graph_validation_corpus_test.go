...
// corpusModels returns the non-skipped cases of the shared semantic corpus with
// their models already parsed.
//
// It fails on a case that does not parse rather than dropping it. The corpus is
// the contract between the implementations, and a case silently excluded from a
// coverage count is worse than one that fails.
...
// TestGraphAgreesWithTraversalOnWhichModelsAreInvalid pins how the two paths
// divide the corpus, and the claim that matters is graphOnly being zero: over
// every case available there is no model the graph rejects that validation calls
// valid. That is what makes the graph safe to put behind the same entry points.
//
// The other counts are here so a change to either path shows up as a number
// rather than as a corpus case quietly moving between buckets.
func TestGraphAgreesWithTraversalOnWhichModelsAreInvalid(t *testing.T) {
	...
	for _, entry := range cases {
		invalidByTraversal := len(findingsOf(ValidateDSL(entry.Model, entry.Case.DSL))) > 0
...
// graphDivergentCorpusCases are the corpus cases that do not pass under the graph
// entry-point phase, and every one of them diverges the same way: the case
// expects one relation-no-entry-point per unreachable relation, and the graph
// path reports a single graph-model-unbuildable for the model.
//
// The cause is the builder, which stops at the first problem and returns no graph
// with its error, so there is nothing left to enumerate relations from.
// Twenty-three positioned findings across these eleven cases become eleven
// positionless ones.
//
// Every other case whose build is refused is stopped by the cascade gate before
// the entrypoint phase runs, so its refusal is never reported and the case still
// passes.
...
// TestCorpusUnderGraphValidation runs the whole corpus with the graph entry-point
// phase selected and pins the divergence to exactly the cases above, by name.
//
// Naming them is the point. A count alone would stay green if one case started
// failing as another started passing, which is the shape a rule-parity regression
// takes.
func TestCorpusUnderGraphValidation(t *testing.T) {
	...
	for _, entry := range corpusModels(t) {
		result := compareWithCorpus(entry.Case.ExpectedErrors,
			findingsOf(ValidateDSLWithGraph(entry.Model, entry.Case.DSL)))
...
// TestGraphDivergenceIsOnlyTheUnbuildableSubstitution checks what the divergence
// above consists of, so the test names a behaviour rather than a list.
//
// For each divergent case: the case expects nothing but entrypoint findings, and
// the graph path reports exactly one finding, the unbuildable one. Anything else,
// a second finding or a different code, is a different problem than fail-fast
// Build.
func TestGraphDivergenceIsOnlyTheUnbuildableSubstitution(t *testing.T) {
	...
	for _, entry := range corpusModels(t) {
		...
		findings := findingsOf(ValidateDSLWithGraph(entry.Model, entry.Case.DSL))

		require.Lenf(t, findings, 1, "case %q reports more than the unbuildable finding", entry.Case.Name)
		assert.Equalf(t, GraphModelUnbuildable, findings[0].Metadata.Kind,
...
// TestEntrypointRuleMatchesTraversalWhereTheGraphBuilds is the parity guarantee
// behind the switch: wherever there is a graph to read, reading it names the same
// unreachable relations as walking the rewrite tree, with the same message and
// the same position.
//
...
func TestEntrypointRuleMatchesTraversalWhereTheGraphBuilds(t *testing.T) {
	...
	for _, entry := range corpusModels(t) {
		...
		fromTraversal := entrypointFindings(entry.Model, entry.Case.DSL, validateEntryPoints)
		fromGraph := entrypointFindings(entry.Model, entry.Case.DSL, validateWithGraph)
...
// carrying the symbol, the position and the message, which is everything the
// corpus compares.
func entrypointFindings(model *openfgav1.AuthorizationModel, dsl string, entryPoints entryPointPhase) []string {
	findings := findingsOf(validateWith(model, newSource(dsl), entryPoints))

	described := []string{}

	for _, finding := range findings {
		if finding.Metadata.Kind != RelationNoEntrypoint {
			continue
		}
