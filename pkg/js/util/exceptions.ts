...
const createInvalidRelationOnTuplesetError = (
  ...
  errors.push(
    constructValidationError({
      message: `the \`${offendingRelation}\` relation definition on type \`${typeDef}\` is not valid: ` +
...
function createMultipleModuleInSingleFileError(props: BaseProps, file: string, modules: string[]) {
  ...
  errors.push(
    constructValidationError({
      message: `file ${file} would contain multiple module definitions (${modules.join(", ")}) ` +
