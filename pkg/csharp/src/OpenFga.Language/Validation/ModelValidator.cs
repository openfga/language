...
public class ModelValidator
{
                        ...
                        else
                        {
                            ...
                            if (allowableTypesResult.Valid && allowableTypesResult.AllowableTypes.Count > 0)
                            {
                                ...
                                foreach (var item in fromTypes)
                                {
                                    ...
                                    {
                                        if (typeMap[decodedType] != null && !typeMap[decodedType].Relations.ContainsKey(childDef.Target))
                                        {
                                            var typeIndex = _dsl.GetTypeLineNumber(typeName);
                                            var lineIndex = _dsl.GetRelationLineNumber(relationName, typeIndex);
    ...
    private void TrackModulesInFile(OpenFga.Sdk.Model.Metadata? metadata)
    {
        if (metadata == null)
        {
            return;
    ...
    private void TrackModulesInFile(OpenFga.Sdk.Model.Metadata? metadata, RelationMetadata? relationMetadata)
    {
        string? module = null;
