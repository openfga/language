...
type CSR struct {
	...
	edgeRecursiveRelations  []int32
	edgeRelationDefinitions []int32
	edgeCycles              []uint64
...
func (csr *CSR) NodeCount() int {
	return int(csr.nodes)
}

func (csr *CSR) EdgeCount() int {
	return int(csr.edges)
}

// NodeLabel returns the label for a given node.
func (csr *CSR) NodeLabel(node int) string {
...
func (csr *CSR) NodeID(node int) string {
...
// for the given value.
func (csr *CSR) NodeFromID(id string) (int, bool) {
...
// value.
func (csr *CSR) MustNodeFromID(id string) int {
...
func (csr *CSR) NodeType(node int) NodeType {
...
func (csr *CSR) NodeRecursiveRelation(node int) (int, bool) {
	recursiveNode := csr.nodeRecursiveRelations[node]
	return int(recursiveNode), recursiveNode >= 0
}
...
// the source node.
func (csr *CSR) NodeWeight(sourceNode int, terminalNode int) (int, bool) {
	start := csr.nodeReachabilityPointers[sourceNode]
	end := csr.nodeReachabilityPointers[sourceNode+1]

	for i := start; i < end; i++ {
		n := int(csr.nodeReachability[i] >> 32)
		w := int(csr.nodeReachability[i] & 0xFFFFFFFF)
...
func (csr *CSR) EdgesFromNode(sourceNode int) iter.Seq[int] {
...
func (csr *CSR) NodeReachesWildcard(sourceNode int, name string) bool {
	...
	for i := start; i < end; i++ {
		if csr.nodeWildcards[i] == int32(wildcardNode) {
			return true
		}
	}
	return false
...
// tuple cycle.
func (csr *CSR) NodeIsCyclic(node int) bool {
	ndx := node / 64
	bit := node % 64
...
func (csr *CSR) EdgeType(edge int) EdgeType {
	return EdgeType(csr.edgeTypes[edge])
}

func (csr *CSR) EdgeSource(edge int) int {
	return int(csr.edgeEndpoints[edge] >> 32)
}

func (csr *CSR) EdgeTarget(edge int) int {
	return int(csr.edgeEndpoints[edge] & 0xFFFFFFFF)
}

func (csr *CSR) EdgeRecursiveRelation(edge int) int {
	return int(csr.edgeRecursiveRelations[edge])
}

func (csr *CSR) EdgeRelationDefinition(edge int) int {
	return int(csr.edgeRelationDefinitions[edge])
}

func (csr *CSR) EdgeIsCyclic(edge int) bool {
	ndx := edge / 64
	bit := edge % 64
	return ((csr.edgeCycles[ndx] >> bit) & 1) != 0
}

func (csr *CSR) EdgeWeight(sourceEdge int, terminalNode int) (int, bool) {
	start := csr.edgeReachabilityPointers[sourceEdge]
	end := csr.edgeReachabilityPointers[sourceEdge+1]

	for i := start; i < end; i++ {
		n := int(csr.edgeReachability[i] >> 32)
		w := int(csr.edgeReachability[i] & 0xFFFFFFFF)
...
func (csr *CSR) EdgeReachesWildcard(sourceEdge int, name string) bool {
	wildcardNode, ok := csr.NodeFromID(name + ":*")
	if !ok {
		return false
	}
	...
	for i := start; i < end; i++ {
		if csr.edgeWildcards[i] == int32(wildcardNode) {
			return true
		}
	}
	return false
...
func (csr *CSR) EdgeConditions(edge int) iter.Seq[string] {
...
func pushNode(csr *CSR, node *WeightedAuthorizationModelNode) int {
	ndx := csr.nodes
	csr.nodes++
...
func pushEdge(csr *CSR, edge *WeightedAuthorizationModelEdge) int {
	ndx := csr.edges
	csr.edges++
	...
	for _, wildcardType := range edge.wildcards {
		...
		csr.edgeWildcards = append(csr.edgeWildcards, int32(targetNode))
	...
	if relation == "" {
		...
		csr.edgeRecursiveRelations[ndx] = int32(csr.MustNodeFromID(relation))
	}

	relation = edge.relationDefinition
	csr.edgeRelationDefinitions[ndx] = int32(csr.MustNodeFromID(relation))
...
func applyNodeWildcards(csr *CSR, node *WeightedAuthorizationModelNode) {
	for _, wildcardType := range node.wildcards {
		targetNode := csr.MustNodeFromID(wildcardType + ":*")
		csr.nodeWildcards = append(csr.nodeWildcards, int32(targetNode))
...
func NewCSR(g *WeightedAuthorizationModelGraph) *CSR {
	...
	csr.idToNode = make(map[string]int32, nodeCount)
	csr.nodeTypes = make([]uint8, nodeCount)
	csr.nodeCycles = make([]uint64, (nodeCount/64)+1)
	csr.nodeRecursiveRelations = make([]int32, nodeCount)
	...
	var edgeWildcardSize int
	var edgeReachabilitySize int
	...
	for _, node := range g.nodes {
		...
		for _, edge := range g.edges[node.uniqueLabel] {
			...
			edgeWildcardSize += len(edge.wildcards)
			edgeReachabilitySize += len(edge.weights)
		}
	}

	csr.nodeLabels = make([]byte, 0, nodeLabelSize)
	csr.nodeIDs = make([]byte, 0, nodeIDSize)
	csr.nodeWildcards = make([]int32, 0, nodeWildcardSize)
	csr.nodeReachability = make([]uint64, 0, nodeReachabilitySize)
	csr.edgeConditions = make([]byte, 0, conditionsSize)
	csr.edgeWildcards = make([]int32, 0, edgeWildcardSize)
	csr.edgeReachability = make([]uint64, 0, edgeReachabilitySize)
	...
	for ndx, node := range nodes {
		...
		csr.nodeRecursiveRelations[ndx] = int32(recursiveNode)
		if !ok {
	...
	csr.edgeRecursiveRelations = make([]int32, edgeCount)
	csr.edgeRelationDefinitions = make([]int32, edgeCount)
...
func (csr *CSR) UnmarshalBinary(b []byte) error {
	...
	copy(csr.nodeTypes, b[:length])
	b = b[length:]

	length = int(binary.BigEndian.Uint32(b))
	b = b[4:]
	csr.nodeRecursiveRelations = make([]int32, length)
	for i := range length {
		csr.nodeRecursiveRelations[i] = int32(binary.BigEndian.Uint32(b))
		b = b[4:]
	}
	...
	for node := range csr.nodes {
		csr.idToNode[csr.NodeID(int(node))] = node
	}
	...
	b = b[4:]
	csr.nodeWildcards = make([]int32, length)
	for i := range length {
		csr.nodeWildcards[i] = int32(binary.BigEndian.Uint32(b))
		b = b[4:]
	}
	...
	length = int(binary.BigEndian.Uint32(b))
	b = b[4:]
	csr.edgeRecursiveRelations = make([]int32, length)
	for i := range length {
		csr.edgeRecursiveRelations[i] = int32(binary.BigEndian.Uint32(b))
		b = b[4:]
	}

	length = int(binary.BigEndian.Uint32(b))
	b = b[4:]
	csr.edgeRelationDefinitions = make([]int32, length)
	for i := range length {
		csr.edgeRelationDefinitions[i] = int32(binary.BigEndian.Uint32(b))
		b = b[4:]
	}
	...
	length = int(binary.BigEndian.Uint32(b))
	b = b[4:]
	csr.edgeWildcards = make([]int32, length)
	for i := range length {
		csr.edgeWildcards[i] = int32(binary.BigEndian.Uint32(b))
		b = b[4:]
	}
