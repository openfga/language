...
// { "typeName": { "relationName": Token } }
type TokenMap = { [key: string]: { [key: string]: antlr.Token } };
const tokenLookup = {} as TokenMap;

function lookupToken(typeName: string, relationName: string): antlr.Token | undefined {
  if (tokenLookup[typeName] && tokenLookup[typeName][relationName]) {
      return tokenLookup[typeName][relationName];
  }
}

function storeLookupToken(typename: string, relationName: string, token: antlr.Token) {
  if (!tokenLookup[typename]) {
    tokenLookup[typename] = {};
  }

  tokenLookup[typename][relationName] = token;
}
  ...
  exitRelationDeclaration = (ctx: RelationDeclarationContext) => {
    ...
    if (relationDef) {
      if (this.currentMixin) {
        ...
        storeLookupToken(typeName, relationName, ctx.relationName().start);
