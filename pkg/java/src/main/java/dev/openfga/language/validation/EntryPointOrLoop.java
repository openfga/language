...
class EntryPointOrLoop {
    ...
    public static EntryPointOrLoop compute(Map<String, TypeDefinition> typeMap, String typeName, String relationName, Userset rewrite, Map<String, Map<String, Boolean>> visitedRecords) {
        ...
        if (rewrite.getThis() != null) {
            ...
            var baseEntryOrLoop = compute(typeMap, typeName, relationName, rewrite.getDifference().getBase(), visited);
            if (!baseEntryOrLoop.hasEntry()) {
                return baseEntryOrLoop;
            }
            var substractEntryOrLoop = compute(typeMap, typeName, relationName, rewrite.getDifference().getSubtract(), visited);
